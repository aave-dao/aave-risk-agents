...
/**
 * @title AaveSupplyCapAgent
 * @author BGD Labs
 * @notice Agent contract to be used by the agentHub to do validation and
 *         injection for supply caps by risk oracle on the Aave protocol
 */
contract AaveSupplyCapAgent is BaseAaveAgent {
  ...
  function _validateUpdate(
    ...
    uint256 currentCap = POOL.getConfiguration(update.market).getSupplyCap();
